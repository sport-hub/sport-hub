...
export async function ImportCompetition(file) {
  ...
  var matches = GetCsvData(
    join(basePathFiles, `${pathParser(file).name} matches.csv`)
  );

  xmlData.League.Event.forEach(event => {
    var divisions = Array.isArray(event.Division)
      ? [...event.Division]
      : [event.Division];
    divisions.forEach(division => {
      var foundFixtures = fixtures.filter(
        x => x.drawid == division.DivisionLPId
      );

      division.Fixture = [
        ...foundFixtures.map(fixture => {
          var foundMatches = matches.filter(
            x => x.teammatchid == fixture.matchid
          );

          var dateParts = `${fixture.plannedtime}`
          ...
          fixture.Match = [
            ...foundMatches.map(match => {
              ...
              var team1 = {
                player: match.team1player1memberid,
                partner: match.team1player2memberid
              };
              var team2 = {
                player: match.team2player1memberid,
                partner: match.team2player2memberid
              };
              if (match.winner == 1) {
                winner = team1;
                loser = team2;
              ...
              if (match.flag_MatchNotPlayedTeam2DidNotShow) {
                winner = team1;
                loser = team2;
              }

              if (match.flag_MatchNotPlayedTeam1DidNotShow) {
                winner = team2;
                loser = team1;
              }
              ...
              var xmlMatch = {
                ...
                MatchPlayer2: "NA",
                MatchWinnerName: "NA",
                MatchWinnerLTANo: winner.player,
                MatchWinnerPartner: "NA",
                MatchWinnerPartnerLTANo: winner.partner,
                MatchLoserName: "NA",
                MatchLoserLTANo: loser.player,
                MatchLoserPartner: "NA",
                MatchLoserPartnerLTANo: loser.partner,
                MatchScore: "NA",
                MatchScoreCode: "NA",
          ...
          var team1 = xmlData.League.Team.find(
            x =>
              x.TeamName.replace("&amp;", "&").indexOf(fixture.team1name) >= 0
          );
          var team2 = xmlData.League.Team.find(
            x =>
              x.TeamName.replace("&amp;", "&").indexOf(fixture.team2name) >= 0
          );
  ...
  writeFileSync(
    join(diretory, year, file),
    create(xmlData, { encoding: "utf-8" }).end({ pretty: true, indent: "    " })
  );
}
...
export function GetCsvData(file) {
  var test = csvParser(GetData(file), { header: true, dynamicTyping: true });
  return test.data;
}
